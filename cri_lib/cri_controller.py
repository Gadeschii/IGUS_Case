...
class CRIController:
    ...
    def are_all_axes_referenced(self, axes=("A1", "A2", "A3","A4", "A5", "A6","E1")) -> bool:
        ...
        -----------
        axes: tuple[str]
            Nombres de ejes como 'A1', 'A2', etc.

        Devuelve:
        ---------
        bool: True si todos están referenciados (estado == REFERENCED), False en caso contrario
        """
        self.get_referencing_info()
        state = self.robot_state.referencing_state
        ...
        for axis in axes:
            axis_state = getattr(state, axis)
            if axis_state != ReferencingAxisState.REFERENCED:
                print(f"❌ {axis} NO referenciado (estado: {axis_state.name})")
                return False
            else:
    ...
    def get_referencing_info(self) -> dict:
        """Consulta y devuelve el estado de referencia de cada eje."""
        self.referencing_status_raw = ""  # Limpiar antes de nueva petición

        if self._send_command("CMD GetReferencingInfo", True, "info_referencing") is not None:
            if self._wait_for_answer("info_referencing", timeout=self.DEFAULT_ANSWER_TIMEOUT) is None:
                # Aquí intenta extraer los datos desde robot_state.parser o equivalente
                with self.robot_state_lock:
                    # Suponiendo que `self.parser.last_referencing_data` contiene la respuesta textual
                    raw = self.parser.last_referencing_data if hasattr(self.parser, "last_referencing_data") else ""
                    self.referencing_status_raw = raw
                    status = {}
                    for line in raw.splitlines():
                        if line.startswith("ReferencingStatus"):
                            parts = line.strip().split()
                            if len(parts) == 3:
                                joint, value = parts[1], parts[2]
                                status[joint] = bool(int(value))
                    return status

        return {}

    
    def get_joint_referencing_status(self) -> dict:
        """Returns a dictionary of joint referencing statuses using CMD GetReferencingInfo."""
        if self._send_command("CMD GetReferencingInfo", True, "info_referencing") is not None:
            if self._wait_for_answer("info_referencing", timeout=self.DEFAULT_ANSWER_TIMEOUT) is None:
                with self.robot_state_lock:
                    # Filtrar solo líneas que contienen estado de referenciado
                    lines = self.robot_state.raw_data.get("info_referencing", "").splitlines()
                    status = {}
                    for line in lines:
                        if line.startswith("ReferencingStatus"):
                            parts = line.strip().split()
    ...
    def get_global_signal(self, id: str):
            """Get global signal
            ...
            command = f"VAR GetNrVariable isFinish"
            msg_id = self._send_command(command, True,"info_var")
            return self._wait_for_answer(
                        "info_var", timeout=30 ##self.DEFAULT_ANSWER_TIMEOUT
                    )
            if (msg_id := self._send_command(command, True)) is not None:
                if (
                    error_msg := self._wait_for_answer(
                        f"{msg_id}", timeout=30 ##self.DEFAULT_ANSWER_TIMEOUT
                    )
                ) is not None:
                    logger.debug("Error in DIN command: %s", error_msg)
                    return False
